...
describe('PDFProcessor', () => {
  ...
  describe('loadPDF', () => {
    ...
    it('should reject invalid PDF files', async () => {
      ...
      await expect(pdfProcessor.loadPDF(invalidFile)).rejects.toThrow(
        'Invalid PDF file'
      );
    });
  });
  ...
  describe('compressImages', () => {
    it('should compress images in a PDF document', async () => {
      ...
      const result = await pdfProcessor.compressImages(
        mockPdfDoc,
        70,
        progressCallback
      );
    ...
    it('should handle compression errors', async () => {
      ...
      await expect(pdfProcessor.compressImages(mockPdfDoc, 70)).rejects.toThrow(
  ...
  describe('removeImages', () => {
    ...
    it('should handle image removal errors', async () => {
      ...
      await expect(pdfProcessor.removeImages(mockPdfDoc)).rejects.toThrow(
  ...
  describe('splitByPages', () => {
    ...
    it('should reject invalid page ranges', async () => {
      ...
      await expect(
        pdfProcessor.splitByPages(mockPdfDoc, 10, 15)
  ...
  describe('splitBySize', () => {
    it('should split a PDF by file size', async () => {
      ...
      const result = await pdfProcessor.splitBySize(
        mockPdfDoc,
        10,
        progressCallback
      );
  ...
  describe('processPDF', () => {
    it('should process a PDF with all options', async () => {
      // Create a mock file
      const mockFile = new File([new ArrayBuffer(1000)], 'test.pdf', {
        type: 'application/pdf',
      });
